...
const PAYWALL_CONFIG = {
  stripePublishableKey: 'pk_test_YOUR_STRIPE_KEY_HERE', // Replace with your Stripe key
  plans: {
    monthly: {
      id: 'monthly',
      name: 'Monthly',
      price: 12,
      period: 'month',
      stripePriceId: 'price_YOUR_MONTHLY_PRICE_ID', // Replace with your Stripe price ID
      features: [
    ...
    annual: {
      ...
      price: 79,
      period: 'year',
      stripePriceId: 'price_YOUR_ANNUAL_PRICE_ID', // Replace with your Stripe price ID
...
function hasAccess() {
  // In demo/build mode — always returns true so docs work during development
  // Replace this with real Stripe verification in production
...
function grantAccess(plan) {
  const duration = plan === 'annual'
    ? 365 * 24 * 60 * 60 * 1000
    : 31 * 24 * 60 * 60 * 1000;
  sessionStorage.setItem(SESSION_KEY, 'granted_' + Date.now());
...
async function startCheckout(planId) {
  ...
  // DEMO MODE — grant access immediately (remove when Stripe is live)
  if (PAYWALL_CONFIG.stripePublishableKey.includes('YOUR_STRIPE_KEY')) {
    grantAccess(planId);
    hidePricingModal();
